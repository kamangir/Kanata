...
function abcli_create_video() {
    ...
    if [ $(abcli_option_int "$options" help 0) == 1 ]; then
        ...
        local options="jpg|png,fps=<10>,filename=<video>,prefix=<prefix>,rm_frames,resize_to=<$ABCLI_VIDEO_DEFAULT_SIZE>,scale=<1>"
    ...
    local index=0
    local size=""
    for filename in $object_path/$prefix*.$extension; do
        abcli_log "🖼️  $filename"
